...
// cuando le de clic necesito que ejecute una función de tipo flecha, esta función no va a mandar a ninguna pagina externa, solo lo va a validar y que apareza un msj: mirar si aqui puedo poner calcular()
formulario.addEventListener('submit', (e) => {
    e.preventDefault(); // para que cuando se de clic y no esten bueno, no envíe nada
...
// Ecuación para frecuencias naturales (w) 
...
//---------------Función multiplicación de matriz x vector--------------------//	
...
//---------------Función para inversa de una matriz-------------------//
...
//--------Polinomio caracterísctico de una matriz (vectores propios)--------//
...
//-------------Método de Bairstow, raíces de polinomio------------------//

// Los coeficientes se colocan desde el término independiente al término de mayor grado.
// Este método presenta un límite iteraciones, para números muy grandes, presenta fallas.
...
var raicespoli = raicespoli.sort(menoramayor);  //organiza de menor a mayor, del primer al último piso. Para part masa deben ir en este sentido

// Las raíces que se hallan del polinomio característico deben ser iguales al número de pisos que se está trabajando y valores positivos. De no ser así, esto indicaría una inconsistencia en los valores propuestos y, por tanto, la función modosnormales no daría un resultado correcto	
...
var w = w.sort(mayoramenor); //organiza de mayor a menor, del último piso al primero porque así se está imprimiendo
    
// Ciclos por segundo (f) y períodos naturales (T)
...
var Modos = modosnormales(matrizA, raicespoli);    // primer piso a último por vectores para part masa
...
//----------Función sumatoria para matriz de amortiguamiento--------------//
...
// FACTOR P QUEDÓ DE PRIMER PISO AL ULTIMO por el orden de los modos más no por el de masa!!!!
...
var Partmasa = new Array(); // Vector con la participación de la masa en kg
var Partmasaxmodo = new Array();
var Partmasaporcent = new Array();
var porcent = 0; // contador que lleva la suma de porcentajes
var porcentajeM; // % de participiación de masa alcanzado
...
// Datos para graficar el espectro sísmico de diseño (AQUÍ SE DEBE INSERTAR AUTOMATICA/)
...
// Valores de pseudoaceleración obtenidos por el espectro (interpolación)

T = T.sort(mayoramenor); // Se debe cambiar el orden ya que el primer modo que participa se refiere la frecuencia natural más pequeña con período más grande, es decir, el primer piso.
...
// Vectores aceleración por cada masa que participa representados como vectores columna
// Queda del primer piso al último

var vectores_a = new Array();

for (i=0; i<n; i++){ // tamaño de fila por numero de pisos 
    vectores_a[i] = new Array(Partmasa.length);
    for (j=0; j<Partmasa.length; j++){ // tamaño de columna por numero de Partmasa
        vectores_a[i][j] = A[j]*Modos[i][j];	// en m/s2
    }
}

// Vector fuerza total donde cada componente corresponde a la fuerza de cada piso (no estoy segura si es así)

var Ftotal = new Array();

// QUEDA DEL PRIMER PISO AL ÚLITMO
...
function show1(context, canvas) {
    ...
    context.fillText('Cálculo de matriz de amortiguamiento', 50, 50);
    ...
    context.fillText('Períodos naturales', x, y);
    ...
    context.fillText('Matriz de modos de vibración', x, y);
...
function show2(context, canvas) {
    ...
    context.fillText('Cálculo de fuerzas y desplazamientos', 50, 50);
    ...
    context.fillText('Factor de participación', x, y);
    ...
    context.fillText('Participación de la masa en cada modo', x, y);
    ...
    for (i=0; i<n; i++){
        ...
        context.fillText('En el modo '+ rango[i] + ' participa el ' + Partmasaporcent[i] + ' % de la masa total del pórtico. ', x, y + longy + esp);
    }

    if (Partmasa.length==1){

        context.fillText('El 90% de la participación de la masa está en el modo '+ Partmasa.length + '.', x, y + esp + 2*longy + 10);

    }else if (Partmasa.length>1){

        context.fillText('El 90% de la participación de la masa está en los '+ Partmasa.length + ' primeros modos.', x, y + esp + 2*longy + 10);
    ...
    // Matriz o vector de aceleración
    ...
    context.fillText('Matriz o vector de aceleración [m/s\u00B2]', x, y);
        ...
        var chart = new Chart (ctx,{
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'Espectro Sísmico de Diseño de la Ciudad de Pasto',
                        color: 'black',
                        font: {
...
// Función que dibuja rectángulos para el movimiento oscilatorio
...
function drawRectangle(myRectangle, context, canvas) {
    ...
    context.fillText('Simulación del movimiento', canvas.width/2 - bplanta/2 ,17);
...
// Función que dibuja el contorno de los rectángulos en posición inicial
...
// Función que permite el movimiento esperado
...
function animate(n, myRectangle, myRectangle1, canvas, context, startTime, amplitude) {
    ...
    // Dibujar líneas punteadas posición inicial
...
// Ciclo que guarda en matriz las coordenadas (x,y) de cada rectángulo del movimiento oscilatorio
...
// Ciclo que guarda en matriz las coordenadas (x,y) de cada rectángulo para dibujo de posición inicial
